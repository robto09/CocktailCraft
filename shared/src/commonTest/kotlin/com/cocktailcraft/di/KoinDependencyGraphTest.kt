...
class KoinDependencyGraphTest : MainDispatcherTest() {
    ...
    @Test
    fun compositeCocktailRepositoryBindingsShareOneInstance() {
        val koin = startTestKoin()

        val repository = koin.get<CocktailRepository>()
        assertSame(repository, koin.get<CocktailSearchRepository>())
        assertSame(repository, koin.get<CocktailDetailRepository>())
        assertSame(repository, koin.get<CocktailCatalogRepository>())
    }
